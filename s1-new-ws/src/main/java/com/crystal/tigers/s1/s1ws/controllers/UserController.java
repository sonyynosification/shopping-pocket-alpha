...
public class UserController {
	...
	public ResponseEntity<User> update(@PathVariable int user_id, @RequestBody User user) {
		...
		if (currentUser == null) {
			LOG.info("com.crystal.tigers.s1.s1ws.dbmodels.User with id {} not found", user_id);
			return new ResponseEntity<User>(HttpStatus.NOT_FOUND);
		}
    ...
    public ResponseEntity<Void> delete(@PathVariable("user_id") int id){
        ...
        if (user == null){
            LOG.info("Unable to delete. com.crystal.tigers.s1.s1ws.dbmodels.User with id {} not found", id);
